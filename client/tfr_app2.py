...
class Application(tk.Frame):
    ...
    def create_initial(self):
        ...
        self.program_list = tk.Listbox(self.initial_frame,
                                       exportselection=0)
    ...
    def populate_active_program_list(self):
    ...
    def create_supporter_button(self):
        ...
        self.supp_by_id_separator = ttk.Separator(self.supporter_frame, 
                                                  orient=tk.HORIZONTAL)
        self.supp_by_id_separator.grid(row=3, column=0, sticky = "ew")

        ### Supporter by id
        self.sup_by_id_frame = tk.Frame(self.supporter_frame, pady=6)
        self.sup_by_id_frame.grid(row=4, column=0)
        # Supporter id 
    ...
    def next_supporter(self):
        if self.operator.chosen_program is None:
            ...
            try:
                ...
                tkMessageBox.showwarning('Active operators', e)
...
def login_popup(master):
    toplevel = tk.Toplevel(master)
    login_form = tk.Frame(self)
    login_form.grid(row=0,column=0)
        
    # Username
    lookupid_lbl = tk.Label(toplevel,
                            font=self.BOLD, text="Lookup ID:")
    lookupid_lbl.grid(row=0, column=0)
        
    lookupid_var = tk.StringVar()
    lookupid_entry = tk.Entry(toplevel,
                              textvariable=self.lookupid_var)
    lookupid_entry.grid(row=0, column=1)

        # Password
    password_lbl = tk.Label(toplevel,
                            font=self.BOLD, text="Password:")
    password_lbl.grid(row=1, column=0)

    password_var = tk.StringVar()
    password_entry = tk.Entry(toplevel, show='*',
                              textvariable=self.password_var)
    password_entry.grid(row=1, column=1)
        
    # OK button
    ok_button = tk.Button(toplevel, text="OK",
                          command=self.password_accept)
...
if __name__=='__main__':
    ...
    # Enable mousewheel for main window scroll
